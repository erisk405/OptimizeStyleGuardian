...
export interface DesignSystemIssue {
    type: 'component_replacement' | 'class_replacement' | 'color_token' | 'spacing_token';
    file: string;
    line: number;
    column?: number;
    current: string;
...
export interface PerformanceIssue {
    type: 'large_bundle' | 'unused_css' | 'missing_lazy_load' | 'missing_dimensions' |
          'sync_script' | 'large_inline_script' | 'heavy_component' | 'expensive_selector' |
          'large_image' | 'missing_viewport' | 'missing_preload' | 'layout_shift_risk' |
          'render_blocking';
    file: string;
    line?: number;
    column?: number;
