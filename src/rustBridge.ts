...
export class RustBridge {
  ...
  async analyze(options: ScanOptions): Promise<AnalysisResult> {
    return new Promise((resolve, reject) => {
      ...
      console.log("[RustBridge] Starting analysis with options:", options);

      const args = [
        "--workspace-root",
        options.workspaceRoot,
        "--similarity",
        (options.similarity || 90).toString(),
      ];
      ...
      if (options.performanceThresholds) {
        const { jsBundleKb, cssSizeKb, imageSizeKb } =
      ...
      if (options.enabledCategories) {
        ...
        if (options.enabledCategories.importAnalysis === false) {
          console.log(
            "[RustBridge] Imports disabled, adding --disable-imports flag",
          );
          args.push("--disable-imports");
        } else {
          console.log("[RustBridge] Imports enabled");
        }
      }
      ...
      child.on("close", (code) => {
        ...
        try {
          ...
          console.log("[RustBridge] Analysis result:", {
            duplicates: result.duplicates?.length || 0,
            designSystem: result.designSystem?.length || 0,
          ...
          reject(
            new Error(`Failed to parse engine output: ${error}\n${stdout}`),
          );
        }
      });
    });
  ...
  async checkEngineAvailability(): Promise<{
    ...
    if (!exists) {
      return {
        ...
        error:
          'Engine binary not found. Run "npm run build:engine" to compile.',
      };
    }
