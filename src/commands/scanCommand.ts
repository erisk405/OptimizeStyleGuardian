  ...
  private async performScan(paths: string[], workspaceRoot: string) {
    ...
    const scanOptions: ScanOptions = {
      ...
      configPath: path.join(
        workspaceRoot,
        config.get("designSystemConfigPath", "config/design-system.yml"),
      ),
    ...
    await vscode.window.withProgress(
      ...
      async (progress) => {
        ...
        try {
          ...
          const enableAi = config.get("enableAiSuggestions", false);

          if (enableAi) {
            const hasApiKey = await this.apiKeyManager.hasApiKey();

            if (hasApiKey) {
              progress.report({ message: "Enhancing with AI suggestions..." });

              try {
                const apiKey = await this.apiKeyManager.getApiKey();
                if (apiKey) {
                  const aiModel = config.get(
                    "aiModel",
                    "claude-sonnet-4-5-20250929",
                  );
                  const maxIssues = config.get("aiMaxIssuesPerScan", 10);
                  const contextLines = config.get("aiContextLines", 5);

                  const anthropicService = new AnthropicService(
                    apiKey,
                    aiModel,
                  );
                  const issueEnhancer = new IssueEnhancer(
                    anthropicService,
                    maxIssues,
                    contextLines,
                  );

                  const enhancedResult = await issueEnhancer.enhanceResult(
                    result,
                    workspaceRoot,
                  );
                ...
                vscode.window.showWarningMessage(
                  "AI enhancement failed. Showing basic results.",
                );
                this.panelManager.showPanel();
                this.panelManager.updateResults(result);
                this.updateImportStatusBar(result);
              ...
              const action = await vscode.window.showInformationMessage(
                "AI suggestions are enabled but no API key is configured.",
                "Configure API Key",
                "Continue without AI",
              );
              ...
              if (action === "Configure API Key") {
                await vscode.commands.executeCommand(
                  "go5StyleGuardian.configureApiKey",
                );
              }
